...
const mutations = {
  ...
  SET_HOUSE_ID(state, houseId) {
    // committed from house/fetchActiveHouse - remove from production - need to find code generator or something
...
const actions = {
    ...
    deleteHouse({dispatch, commit, getters}) {
      ...
      console.log('meausure: ', memberCount);
      ...
      globalAxios.get('/users/' + userId + '/house' + '.json' + '?auth=' + token)
        .then((response) => {
          ...
          if (response.data.active) {
            ...
            commit('user/SET_BELONGS_TO_HOUSE', true, gObj_hasRoot);
            commit('house/SET_HOUSE_ID', houseId, gObj_hasRoot);
